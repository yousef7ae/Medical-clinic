                ...
                @if($posts->count() > 0)
                    <table class="table table-borderless mb-md-5">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="text-center p-1">
                                <div class="bg-primary rounded-2 text-white py-2 px-1">{{__("Name Add Post")}}</div>
                            </th>
                            <th scope="col" class="text-center p-1">
                        ...
                        @foreach($posts as $key => $post)
                            <tr>
                                ...
                                <td class="text-center p-1">
                                    <div class="table-os">
                                        <img width="50" class="img-fluid rounded"
                                             src="{{ $post->image ? $post->image : url('dashboard/img/image1.png')}}"
                                ...
                                <td class="text-center p-1">
                                    <div class="table-os">{!!$post->description!!}</div>
                                </td>

                                <td class="text-center p-1">
                                    <div class="table-os">{{ $post->user ? $post->user->name : ''  }}</div>
                                </td>
...
@if(auth()->user()->can('posts create') )
    <!--  Modal CreatePost -->
        <div class="modal fade" wire:ignore.self id="CreatePost" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content px-3 bg-light rounded-4">
                    ...
                    <div class="text-center text-primary border-bottom">
                        <h5 class="" id="exampleModalLabel">{{__("Create Post")}}</h5>
                    </div>
                    <div>
                        <div wire:loading>
                    ...
                    @if($create_post)
                        @livewire('admin.posts.posts-create',[$post_id])
                    @endif
...
@if(auth()->user()->can('posts edit') )
    <!--  Modal EditPost -->
        <div class="modal fade" wire:ignore.self id="EditPost" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content px-3 bg-light rounded-4">
                    <div class="modal-header border-0 py-0">
                    ...
                    <div class="text-center text-primary border-bottom">
                        <h5 class="" id="exampleModalLabel">{{__("Edit Post")}}</h5>
                    </div>
                    <div>
                        <div wire:loading>
                            <i class="fas fa-sync fa-spin"></i>
