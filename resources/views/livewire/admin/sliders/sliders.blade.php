<div>
    ...
    <div class="container-fluid py-3">
        ...
        <div class="main h-100">
            <h2 class="text-primary">{{__("Sliders")}}</h2>
            <div class="row g-0 mb-3">
                <div class="col-md-3 align-self-end">
                    <div class="d-inline">
                        @if(auth()->user()->can('sliders create') )
                            <a class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#CreateSlider"
                               wire:click.prevent="CreateSlider" data-bs-original-title="" title=""><i
                                    class="fa-solid fa-user-plus pe-1"></i> {{__("Create Slider")}} </a>
                        @endif
                    </div>
                </div>
                <div class="col-md-9">
                        ...
                        @foreach($sliders as $key => $slider)
                            <tr>
                                <td class="text-center p-1">
                                    <div class="table-os">{{ $key + $sliders->firstItem() }}</div>
                                </td>
                                <td class="text-center p-1">
                                    <div class="table-os">
                                        <img width="50" class="img-fluid rounded"
                                             src="{{ $slider->image ? $slider->image : url('dashboard/img/image1.png')}}"
                                             data-holder-rendered="true">
                                    </div>
                                </td>
                                <td class="text-center p-1">
                                    <div class="table-os">{{$slider->name}}</div>
                                </td>
                                <td class="text-center p-1">
                                    <div class="table-os">{{ Str::limit($slider->description,50) }}</div>
                                </td>
                                    ...
                                    <div class="table-os">
                                        ...
                                        @if(auth()->user()->can('sliders edit') )
                                            <a class="btn btn-sm mx-1 btn-info text-white border-end" href="#"
                                               wire:click.prevent="EditSlider({{$slider->id}})"
                                        ...
                                        @if(auth()->user()->can('sliders delete') )
                                            <a class="btn btn-sm mx-1 btn-danger" href="#"
                                               ...
                                               data-bs-toggle="modal" data-bs-target="#deleteModalSlider">
...
@if(auth()->user()->can('sliders create') )
    <!--  Modal CreateSlider -->
        <div class="modal fade" wire:ignore.self id="CreateSlider" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content px-3 bg-light rounded-4">
                    ...
                    <div class="text-center text-primary border-bottom">
                        <h5 class="" id="exampleModalLabel">{{__("Create Slider")}}</h5>
                    </div>
                    <div>
                        <div wire:loading>
                    ...
                    @if($create_slider)
                        @livewire('admin.sliders.sliders-create',[$slider_id])
                    @endif
...
@if(auth()->user()->can('sliders edit') )
    <!--  Modal EditSlider -->
        <div class="modal fade" wire:ignore.self id="EditSlider" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content px-3 bg-light rounded-4">
                    <div class="modal-header border-0 py-0">
                    ...
                    <div class="text-center text-primary border-bottom">
                        <h5 class="" id="exampleModalLabel">{{__("Edit Slider")}}</h5>
                    </div>
                    <div>
                        <div wire:loading>
                            <i class="fas fa-sync fa-spin"></i>
...
@if(auth()->user()->can('sliders delete') )
    <!-- Modal deleteModalSlider -->
        <div wire:ignore.self class="modal fade" id="deleteModalSlider" tabindex="-1" role="dialog"
             aria-labelledby="deleteModalSliderLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModalSliderLabel">{{__("Delete Confirm")}}</h5>
