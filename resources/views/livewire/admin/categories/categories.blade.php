<div>
    ...
    <div class="container-fluid py-3">
        ...
        <div class="main h-100">
            <h2 class="text-primary">{{__("Categories")}}</h2>
            <div class="row g-0 mb-3">
                <div class="col-md-3 align-self-end">
                    <div class="d-inline">
                        @if(auth()->user()->can('categories create') )
                            <a class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#CreateCategory"
                               wire:click.prevent="CreateCategory" data-bs-original-title="" title=""><i
                                    class="fa-solid fa-user-plus pe-1"></i> {{__("Create Category")}} </a>
                        @endif
                    </div>
                </div>
                <div class="col-md-9">
                    <form class="row g-1 justify-content-end" wire:submit.prevent="search">
                        ...
                        <div class="col-md-4 col-sm-7">
                            <label class="text-primary mb-1" for="PatientName">{{__("Clinic Name")}}</label>
                        ...
                        <div class="col-md-4 col-sm-7">
                            <label class="text-primary mb-1" for="PatientName">{{__("Mobile Category")}}</label>
                            <input type="text" class="form-control border-primary" wire:model.defer="mobile"
                                   placeholder="{{__("Mobile Category")}} "
                                   id="PatientName">
                        </div>
...
{{--                            </th>--}}
                            <th scope="col" class="text-center p-1">
                                <div class="bg-primary rounded-2 text-white py-2 px-1">{{__("Clinic Name")}}</div>
                            </th>
                            <th scope="col" class="text-center p-1">
                                <div class="bg-primary rounded-2 text-white py-2 px-1">{{__("Mobile Category")}}</div>
                            </th>
                            <th scope="col" class="text-center p-1">
                                <div class="bg-primary rounded-2 text-white py-2 px-1">{{__("Section Manager")}}</div>
                            </th>
                            <th scope="col" class="text-center p-1">
                        ...
                        @foreach($categories as $key => $category)
                            <tr>
                                <td class="text-center p-1">
                                    <div class="table-os">{{ $key + $categories->firstItem() }}</div>
                                </td>
{{--                                <td class="text-center p-1">--}}
{{--                                    <div class="table-os">--}}
{{--                                        <img width="50" class="img-fluid rounded"--}}
{{--                                             src="{{ $category->image ? $category->image : url('dashboard/img/image1.png')}}"--}}
{{--                                             data-holder-rendered="true">--}}
{{--                                    </div>--}}
{{--                                </td>--}}
                                <td class="text-center p-1">
                                    <div class="table-os">{{$category->name}}</div>
                                </td>
                                <td class="text-center p-1">
                                    <div class="table-os">{{$category->mobile}}</div>
                                </td>
                                <td class="text-center p-1">
                                    <div class="table-os">{{ $category->user ? $category->user->name : __("Nothing") }}</div>
                                </td>
                                    ...
                                    <div class="table-os">
                                        ...
                                        @if(auth()->user()->can('categories delete') )
                                            <a class="btn btn-sm mx-1 btn-danger" href="#"
                                               ...
                                               data-bs-toggle="modal" data-bs-target="#deleteModalCategory">
...
@if(auth()->user()->can('categories create') )
    <!--  Modal CreateCategory -->
        <div class="modal fade" wire:ignore.self id="CreateCategory" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content px-3 bg-light rounded-4">
                    ...
                    <div class="text-center text-primary border-bottom">
                        <h5 class="" id="exampleModalLabel">{{__("Create Category")}}</h5>
                    </div>
                    <div>
                        <div wire:loading>
                    ...
                    @if($create_category)
                        @livewire('admin.categories.categories-create',[$category_id])
                    @endif
...
@if(auth()->user()->can('categories edit') )
    <!--  Modal EditCategory -->
        <div class="modal fade" wire:ignore.self id="EditCategory" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content px-3 bg-light rounded-4">
                    <div class="modal-header border-0 py-0">
                    ...
                    <div class="text-center text-primary border-bottom">
                        <h5 class="" id="exampleModalLabel">{{__("Edit Category")}}</h5>
                    </div>
                    <div>
                        <div wire:loading>
                            <i class="fas fa-sync fa-spin"></i>
...
@if(auth()->user()->can('categories delete') )
    <!-- Modal deleteModalCategory -->
        <div wire:ignore.self class="modal fade" id="deleteModalCategory" tabindex="-1" role="dialog"
             aria-labelledby="deleteModalCategoryLabel" aria-hidden="true">
