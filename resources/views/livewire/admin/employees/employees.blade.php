<div>
    ...
    <div class="container-fluid py-3">
        ...
        <div class="main h-100">
            <h2 class="text-primary">{{__("All Employees")}}</h2>
            <div class="row g-0 mb-3">
                <div class="col-md-3 align-self-end">
                    <div class="d-inline">
                        @if(auth()->user()->can('users create') )
                            <a class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#CreateUser"
                               wire:click.prevent="CreateUser" data-bs-original-title="" title=""><i
                                    class="fa-solid fa-user-plus pe-1"></i>{{__("Add a new employee")}}  </a>
                        @endif
                    </div>
                </div>
                <div class="col-md-9">
                    <form class="row g-1 justify-content-end" wire:submit.prevent="search">
                        <div class="col-md-4 col-sm-7">
                            <label class="text-primary mb-1" for="PatientName">{{__("Employee Name")}}</label>
                ...
                @if($users->count() > 0)
                    <table class="table table-borderless mb-md-5">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="text-center p-1">
                                <div class="bg-primary rounded-2 text-white py-2 px-1"> {{__("job")}}</div>
                            </th>
                            <th scope="col" class="text-center p-1">
                                <div class="bg-primary rounded-2 text-white py-2 px-1">{{__("clinic")}}</div>
                            </th>
                            <th scope="col" class="text-center p-1">
                                <div class="bg-primary rounded-2 text-white py-2 px-1">{{__("The Role")}}</div>
                            </th>
                            <th scope="col" class="text-center p-1">
                        ...
                        @foreach($users as $key => $user)
                            <tr>
                                ...
                                <td class="text-center p-1">
                                    <div class="table-os">{{$user->job}}</div>
                                </td>
                                <td class="text-center p-1">
            ...
            <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content px-3 bg-light rounded-4">
                    ...
                    <div class="text-center text-primary border-bottom">
                        <h5 class="" id="exampleModalLabel">{{__("Nothing")}}}} </h5>
                    </div>
                    <div>
                        <div wire:loading>
                    ...
                    @if($create_user)
                        @livewire('admin.employees.employees-create',[$user_id])
                    @endif
