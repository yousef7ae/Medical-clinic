...
class PrescriptionsEdit extends Component
{
    use WithFileUploads;

    public $prescription, $categories, $doctors, $patients, $file;

    function mount($id)
    {
        $prescription = Prescription::findOrFail($id);
        $this->prescription = $prescription->toArray();
    ...
    public function update()
    {
        ...
        if (!empty($this->prescription['file']) and !is_string($this->prescription['file'])) {
            ...
            unset($this->prescription['file']);
        }

        $prescription = Prescription::findOrFail($this->prescription['id']);
        $prescription->update($this->prescription);
        $this->emit('success', __("Updated successfully"));
    }

    public function render()
    {
        return view('livewire.admin.prescriptions.prescriptions-edit')->layout('layouts.admins.app');
    }
}
